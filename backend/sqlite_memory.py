...
class SQLiteConversationMemory:
    ...
    def _connect(self) -> sqlite3.Connection:
        ...
        (via contextlib.closing) so it's closed even if an exception is raised."""
    ...
    def get_history(self, conversation_id: str) -> List[Tuple[str, str]]:
        ...
        try:
            with contextlib.closing(self._connect()) as conn, conn:
                cursor = conn.cursor()
                # Order by id (strictly increasing on insert), not timestamp: SQLite's
                # CURRENT_TIMESTAMP only has 1-second resolution, so rapid successive
                # messages can tie and come back in the wrong order if sorted by time alone.
                cursor.execute('''
